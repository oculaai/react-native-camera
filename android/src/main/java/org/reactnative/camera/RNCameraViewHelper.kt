...
import org.reactnative.camera.events.CameraReadyEvent.Companion.obtain
...
object RNCameraViewHelper {
    ...
    fun emitMountErrorEvent(view: ViewGroup, error: String?) {
        ...
        reactContext.runOnNativeModulesQueueThread {
            val event = CameraMountErrorEvent.obtain(view.id, error)
    ...
    fun emitPictureSavedEvent(view: ViewGroup, response: WritableMap?) {
        val reactContext = view.context as ReactContext
        reactContext.runOnNativeModulesQueueThread {
            val event = PictureSavedEvent.obtain(view.id, response)
            reactContext.getNativeModule(UIManagerModule::class.java)?.eventDispatcher?.dispatchEvent(
                event
            )
        }
    ...
    fun emitPictureTakenEvent(view: ViewGroup) {
        val reactContext = view.context as ReactContext
        reactContext.runOnNativeModulesQueueThread {
            val event = PictureTakenEvent.obtain(view.id)
            reactContext.getNativeModule(UIManagerModule::class.java)?.eventDispatcher?.dispatchEvent(
                event
            )
        }
    ...
    fun emitRecordingEndEvent(view: ViewGroup) {
        val reactContext = view.context as ReactContext
        reactContext.runOnNativeModulesQueueThread {
            val event = RecordingEndEvent.obtain(view.id)
            reactContext.getNativeModule(UIManagerModule::class.java)?.eventDispatcher?.dispatchEvent(
                event
            )
        }
    ...
    fun emitTouchEvent(view: ViewGroup, isDoubleTap: Boolean, x: Int, y: Int) {
        val reactContext = view.context as ReactContext
        reactContext.runOnNativeModulesQueueThread {
            val event = obtain(view.id, isDoubleTap, x, y)
            reactContext.getNativeModule(UIManagerModule::class.java)?.eventDispatcher?.dispatchEvent(
                event
            )
        }
    ...
    fun emitFaceDetectionErrorEvent(view: ViewGroup, faceDetector: RNFaceDetector?) {
        val reactContext = view.context as ReactContext
        reactContext.runOnNativeModulesQueueThread {
            val event = FaceDetectionErrorEvent.obtain(view.id, faceDetector)
            reactContext.getNativeModule(UIManagerModule::class.java)?.eventDispatcher?.dispatchEvent(
                event
            )
        }
    ...
    fun emitBarcodesDetectedEvent(
        ...
        val reactContext = view.context as ReactContext
        reactContext.runOnNativeModulesQueueThread {
            val event = BarcodesDetectedEvent.obtain(view.id, barcodes, compressedImage)
            reactContext.getNativeModule(UIManagerModule::class.java)?.eventDispatcher?.dispatchEvent(
                event
            )
        }
    ...
    fun emitBarcodeDetectionErrorEvent(view: ViewGroup, barcodeDetector: RNBarcodeDetector?) {
        val reactContext = view.context as ReactContext
        reactContext.runOnNativeModulesQueueThread {
            val event = BarcodeDetectionErrorEvent.obtain(view.id, barcodeDetector)
            reactContext.getNativeModule(UIManagerModule::class.java)?.eventDispatcher?.dispatchEvent(
                event
            )
        }
    ...
    fun emitBarCodeReadEvent(
        ...
        val reactContext = view.context as ReactContext
        reactContext.runOnNativeModulesQueueThread {
            val event = BarCodeReadEvent.obtain(view.id, barCode, width, height, compressedImage)
            reactContext.getNativeModule(UIManagerModule::class.java)?.eventDispatcher?.dispatchEvent(
                event
            )
        }
    ...
    fun emitTextRecognizedEvent(view: ViewGroup, data: WritableArray?) {
        val reactContext = view.context as ReactContext
        reactContext.runOnNativeModulesQueueThread {
            val event = TextRecognizedEvent.obtain(view.id, data)
            reactContext.getNativeModule(UIManagerModule::class.java)?.eventDispatcher?.dispatchEvent(
                event
            )
        }
    ...
    private fun getCamcorderProfileQualityFromCameraModuleConstant(quality: Int): Int {
        when (quality) {
            CameraModule.VIDEO_2160P -> {
                return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP) {
                    CamcorderProfile.QUALITY_2160P
                } else CamcorderProfile.QUALITY_1080P
            }
            CameraModule.VIDEO_1080P -> return CamcorderProfile.QUALITY_1080P
            CameraModule.VIDEO_720P -> return CamcorderProfile.QUALITY_720P
            CameraModule.VIDEO_480P -> return CamcorderProfile.QUALITY_480P
            CameraModule.VIDEO_4x3 -> return CamcorderProfile.QUALITY_480P
        }
    ...
    fun setExifData(exifInterface: ExifInterface, exifMap: ReadableMap) {
                ...
                when (tagInfo[0]) {
                    ...
                    "double" -> {
                        exifInterface.setAttribute(
                            name,
                            exifMap.getDouble(name).toString()
                        )
        ...
        if (exifMap.hasKey(ExifInterface.TAG_GPS_LATITUDE) && exifMap.hasKey(ExifInterface.TAG_GPS_LONGITUDE)) {
            exifInterface.setLatLong(
                exifMap.getDouble(ExifInterface.TAG_GPS_LATITUDE),
